module.exports = {
   ...
   * @param  {object}   options - layer (number), bypassProcessing (boolean)
   * @param  {function} callback - gets stored object or `{ status: 'processing' }` if processing
   */
  select: function (key, options, callback) {
    key = key + ':' + (options.layer || 0)

    if (!this.store[key]) return callback(new Error('Not found'))

    var isProcessing = this.store[key].info && this.store[key].info.status === 'processing'
    if (isProcessing && !options.bypassProcessing) {
      return callback(null, [{ status: 'processing' }])
    }
  ...
  getInfo: function (key, callback) {
    if (!this.store[key] || !this.store[key].info) callback(new Error('Info not found'))
