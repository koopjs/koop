...
function setGeomType (json, feature) {
  ...
  if (isPolygon) {
    ...
    json.drawingInfo.renderer = require(__dirname + tplDir + 'renderers/polygon.json')
  } else if (isLine) {
    json.geometryType = 'esriGeometryPolyline'
    json.drawingInfo.renderer = require(__dirname + tplDir + 'renderers/line.json')
  } else {
    json.geometryType = 'esriGeometryPoint'
    json.drawingInfo.renderer = require(__dirname + tplDir + 'renderers/point.json')
  }
...
function info (data, layer, params, callback) {
  ...
  if (layer !== undefined) {
    ...
    json = processTemplate('/templates/featureLayer.json', data, params)
    ...
    json = processTemplate('/templates/featureService.json', (data && data[0]) ? data[0] : data, params)
...
function layers (data, params, callback) {
  ...
  if (!data.length) {
    layerJson = processTemplate('/templates/featureLayer.json', data, params)
    ...
    data.forEach(function (layer, i) {
      layerJson = processTemplate('/templates/featureLayer.json', layer, params)
      layerJson.id = i
...
function queryData (data, params, callback) {
  ...
  if (params.objectIds) {
    ...
    var json = processTemplate(tplDir + 'featureSet.json', data, params)
...
function queryIds (data, params, callback) {
  var json = processTemplate('/templates/featureSet.json', data, params)
