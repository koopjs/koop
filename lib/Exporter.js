          ...
          if ( format === 'zip' ){
            ...
            mkdirp( paths.base, function(){
              ...
              var createZip = function(){
                ...
                // not sure why, but have to be in the directory for the include parameter to work with globbing
                var cmd = ['cd', paths.base + '/', '&&', 'zip', '-rj', '"'+newZipTmp+'"', '.', '--include=' + paths.tmpName + '.*', '--exclude=*.json*'].join(' ');
                console
                exec(cmd, function(err){
                  moveFile(newZipTmp, newZip, callback);
                  removeShapeFile(paths.base + '/', paths.tmpName, function (err){
                    console.log(err);
                  })
                });
              };
...
function moveShapeFile( file, base, name, callback){
  shapefileParts.forEach(function(type) {
    var source = file.replace('zip', type)
    mv(source, base + '/' + name + '.' + type, function(err) {
...
function moveLargeShapeFile( file, base, tmpName, name, callback){
  ...
  shapefileParts.forEach(function(type) {
    mv(shpdir + '/OGRGeoJSON' + '.' + type, base + '/' + name + '.' + type, function (err) {
      if (err) return callback(err)
    })
  });
  callback(null);
