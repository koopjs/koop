...
// an event emitter used to bind to events that need access to koop.db
/*var Emitter = function() {};
Emitter.prototype = Object.create(require('events').EventEmitter.prototype);
var exportEvents = new Emitter();
exports.emitter = exportEvents;*/

// exports large data via multi part file strategy
exports.exportLarge = function( koop, format, id, key, type, options, done ){
  // exports large data via multi part file strategy
  if (!format){
    ...
    fs.writeFile(file, JSON.stringify(json), function(){
      ...
      if (fileCount == pages){
        ...
        fs.writeFile(paths.rootVrtFile, vrt, function(){
          // CALL OGR
          _callOgr(format, paths.rootNewFileTmp, function(err, formatFile){
    ...
    if (fs.existsSync(paths.rootVrtFile) && !options.ignore_cache) {
      ...
      koop.Cache.getInfo(table, function(err, info){
        ...
        _update( info, function(err, res){
          ...
          done(null, {paths: paths, file: file);
...
function callOgr(params, geojson, options, callback){
  ...
  if ( fs.existsSync( outFile ) ) {
    ...
    exec( ogrParams, function (err, stdout, stderr) {
        if (err) {
          ...
          if ( format == 'zip' ){
            ...
            mkdirp( paths.base, function(){
              if ( options.name ){
                ...
                moveShapeFile(outFile, paths.base, options.name, function(){
                  var newZipTmp = paths.base + '/' + options.name + paths.tmpName + '.zip';
                  var newZip =  paths.base + '/' + options.name + '.zip';
                  exec(['zip', '-rj', '"'+newZipTmp+'"', paths.base+'/'].join(' '), function(err, stdout, stderr){
