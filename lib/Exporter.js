              ...
              var createZip = function(){
                ...
                exec(cmd, function(err){
                  ...
                  removeShapeFile(paths.base, options.name, function (err){
                    if (err) { 
                      koop.log.error('error removing shpfile ' + err);
                    }
                  })
                });
              };
...
function moveShapeFile( file, base, name, callback){
  ...
  async.each(shapefileParts, function(type, callback) {
    ...
    moveFile(shpdir + '/OGRGeoJSON' + '.' + type, base + '/' + name + '.' + type, function (err) {
      console.log(shpdir + '/OGRGeoJSON' + '.' + type, base + '/' + name + '.' + type, err)
      callback(err);
    })
  }, function(err) {
    callback(err);
...
function getOgrParams( format, inFile, outFile, geojson, options ){
  ...
  if (format === 'csv') {
    ...
    if (wkid) {
      var proj = projCodes.lookup(wkid);
      if (proj && proj.wkt) {
        cmd.push('-t_srs');
        cmd.push('\''+ fixWkt(proj.wkt, wkid) +'\'');
      } else {
        console.log('No proj info found for WKID', wkid, outFile);
      }
    } else if (wkt) {
      cmd.push('-t_srs');
      cmd.push('\''+ fixWkt(wkt) +'\'');
