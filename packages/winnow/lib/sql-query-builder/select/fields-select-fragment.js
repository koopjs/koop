...
function selectFieldsAsEsriJson (options) {
  ...
  if (fields) {
    return `pickAndEsriFy(properties, geometry, "${fields.join(',')}", "${delimitedDateFields}", "${includeIdField}", "${idField}") as attributes`
  }
  return `esriFy(properties, geometry, "${delimitedDateFields}", "${includeIdField}", "${idField}") as attributes`
}
