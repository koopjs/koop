...
const toGeohash = require('../../../../lib/filter-and-transform/transforms/to-geohash')
const modulePath = '../../../../lib/filter-and-transform/transforms/to-geohash'
...
test('toGeohash, non-point input, returns geohash', t => {
  ...
  const result = toGeohash({ type: 'Polygon', coordinates: 'polygon-coordinates' })
  t.equals(result, 'geohash-result')
  t.ok(geohashSpy.calledOnce)
  t.deepEquals(geohashSpy.firstCall.args, [19, 11, 8])
  t.ok(centroidSpy.calledOnce)
  t.deepEquals(centroidSpy.firstCall.args, [{ type: 'Polygon', coordinates: 'polygon-coordinates' }])
  t.end()
})
