...
test('normalize-query-options, source-data-spatial-reference: known spatial reference', t => {
  t.plan(1)
  const spatialRef = normalizeSourceDataSpatialReference({ inputCrs: 3857 })
  t.equal(spatialRef, 'EPSG:3857')
})

test('normalize-query-options, source-data-spatial-reference: wkt spatial reference', t => {
  t.plan(1)
  const spatialRef = normalizeSourceDataSpatialReference({ inputCrs: 2228 })
...
test('normalize-query-options, source-data-spatial-reference: known spatial reference', t => {
  t.plan(1)
  const spatialRef = normalizeSourceDataSpatialReference({ sourceSR: 3857 })
  t.equal(spatialRef, 'EPSG:3857')
})
...
test('normalize-query-options, source-data-spatial-reference: known spatial reference', t => {
  ...
  const spatialRef = normalizeSourceDataSpatialReference(params)
  t.equal(spatialRef, 'EPSG:4326')
})
