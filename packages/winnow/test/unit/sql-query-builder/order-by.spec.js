...
const createOrderByClause = require('../../../lib/sql-query-builder/order-by')
...
test('createOrderByClause: returns orderBy clause for esri JSON', t => {
  t.plan(1)
  const orderByClause = createOrderByClause({ esri: true, order: ['foo', 'bar'] })
...
test('createOrderByClause: returns orderBy clause with directions ', t => {
  t.plan(1)
  const orderByClause = createOrderByClause({ order: ['foo DESC', 'bar DESC'] })
...
test('createOrderByClause: returns orderBy clause formated for aggregation select', t => {
  t.plan(1)
  const orderByClause = createOrderByClause({ order: ['foo DESC', 'bar DESC'], aggregates: [{ name: 'foo' }] })
  t.equals(orderByClause, ' ORDER BY `foo` DESC, properties->`bar` DESC')
})

test('createOrderByClause: returns orderBy clause formated for aggregation select', t => {
  t.plan(1)
  const orderByClause = createOrderByClause({ order: ['foo DESC', 'bar DESC'], aggregates: [{ name: 'hello' }] })
  t.equals(orderByClause, ' ORDER BY properties->`foo` DESC, properties->`bar` DESC')
})
