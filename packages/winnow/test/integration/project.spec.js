...
test('Project a polygon to Web Mercator using 3857 and translating to esri', (t) => {
  ...
  t.deepEqual(results[0].geometry.rings, [
    [
      ...
      [-11408073.597505985, 6985732.889038823],
...
test('Project to Web Mercator using WKT', (t) => {
  ...
  const options = {
    projection:
      'PROJCS["WGS_1984_Web_Mercator_Auxiliary_Sphere",GEOGCS["GCS_WGS_1984",DATUM["D_WGS_1984",SPHEROID["WGS_1984",6378137.0,298.257223563]],PRIMEM["Greenwich",0.0],UNIT["Degree",0.017453292519943295]],PROJECTION["Mercator_Auxiliary_Sphere"],PARAMETER["False_Easting",0.0],PARAMETER["False_Northing",0.0],PARAMETER["Central_Meridian",0.0],PARAMETER["Standard_Parallel_1",0.0],PARAMETER["Auxiliary_Sphere_Type",0.0],UNIT["Meter",1.0]]', // eslint-disable-line
    limit: 1,
  };
  const results = winnow.query(features, options);
  t.deepEqual(results[0].geometry.coordinates, [-11682713.391976157, 4857924.005275469]);
...
AUTHORITY["EPSG","102645"]]`,
  ...
  const {
    crs,
    features: [feature],
  } = winnow.query(geojson102645, options);
  t.deepEqual(feature.geometry, {
    type: 'Point',
    coordinates: [-13153229.775672015, 4053009.002177773],
  });
  t.deepEquals(crs, {
    type: 'name',
...
test('Project data with CRS 102645 to 2285 using wkid', (t) => {
  ...
  t.deepEquals(crs, {
    type: 'name',
    properties: { name: 'urn:ogc:def:crs:EPSG::2285' },
  });
});
