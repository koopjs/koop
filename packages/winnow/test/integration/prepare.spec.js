...
test('compiling with several and statements', (t) => {
  try {
    t.ok(
      Winnow.prepareQuery({
        where: 'ELEVATION >= 1165 AND ELEVATION <= 4365 AND POP1990 >= 8247 AND POP1990 <= 5700236',
      }),
    );
    t.end();
  } catch (e) {
...
test('prepare query with geometry filter & sourceSR, execute on non-4326 source data', (t) => {
  t.plan(3);
  const options = {
    geometry: {
