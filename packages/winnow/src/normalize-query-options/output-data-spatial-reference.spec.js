...
test('defer to projection option', (t) => {
  t.plan(1);
  const options = {
    projection: 4326,
    srsname: 4269,
...
test('defer to srsname option', (t) => {
  t.plan(1);
  const options = {
    srsname: 4269,
    srsName: 3857,
...
test('defer to inputCrs option', (t) => {
  t.plan(2);
  const options = {
    inputCrs: 2227,
  };
...
test('defer to sourceSR option', (t) => {
  t.plan(2);
  const options = {
    sourceSR: 2227,
  };
...
test('defer to collection.crs option', (t) => {
  t.plan(2);
  const options = {
    collection: {
