...
function normalizeSourceDataSpatialReference({
  inputCrs,
  sourceSR,
  collection,
} = {}) {
  const sourceDataSpatialReference =
    inputCrs || sourceSR || getCollectionCrs(collection);
  if (!sourceDataSpatialReference) return { wkid: 4326 };

  const spatialReference = normalizeSpatialReference(
    sourceDataSpatialReference,
  );

  if (!spatialReference) {
    logManager.logger.debug(
      `spatial reference "${sourceDataSpatialReference}" could not be normalized. Defaulting to EPSG:4326.`,
    );
    // @TODO: throw error?
  }
