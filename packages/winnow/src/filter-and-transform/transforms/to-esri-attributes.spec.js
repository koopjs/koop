...
test('toEsriAttributes, requires idField, properties contain it, but logs range message', (t) => {
  ...
  t.deepEquals(result, { foo: 'bar', OBJECTID: -1 });
  t.end();
});
...
test('toEsriAttributes, does not require idField, has dateFields', (t) => {
  ...
  const result = toEsriAttributes(
    { hello: 'world', foo: date1.toISOString(), bar: date2.toISOString() },
    { coordinates: [-118, 34] },
    'foo,bar',
    'false',
    '',
...
test('toEsriAttributes, does not require idField, has null dateField', (t) => {
  ...
  const date2 = new Date(1000);
  const result = toEsriAttributes(
    { hello: 'world', foo: null, bar: date2.toISOString() },
    { coordinates: [-118, 34] },
    'foo,bar',
    'false',
    '',
