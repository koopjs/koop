...
describe('Query operations', () => {
  before(function () {
    ...
    const textOIDResponse = FeatureServer.query(dataWithTextOID, {
      outFields: 'OBJECTID',
    });
    this.textObjectIds = textOIDResponse.features.map(
      (feat) => feat.attributes.OBJECTID,
    );
  });
  ...
  it('should return the expected response schema for an optionless query', () => {
    ...
    featuresSchemaOverride
      .validate(response, { presence: 'required' })
      .should.not.have.property('error');
  });
  ...
  describe('non-WGS84 input dataset', function () {
    it('should translate the data properly when geojson.crs is defined', function () {
      const response = FeatureServer.query(taggedNonWGS84, {
        limit: 1,
        returnGeometry: true,
      });
    ...
    it('should translate the data properly req.inputCrs is defined', function () {
      ...
      const response = FeatureServer.query(untaggedNonWGS84, {
        limit: 1,
        returnGeometry: true,
      });
      response.features.length.should.equal(1);
  ...
  describe.skip('when getting featureserver features by id queries', function () {
    ...
    it('should work with string id features', function () {
      const response = FeatureServer.query(dataWithTextOID, {
        objectIds: this.textObjectIds.slice(0, 2).join(','),
      });
      response.should.be.an.instanceOf(Object);
      response.fields.should.be.an.instanceOf(Array);
  ...
  describe('when filtering features with where clauses', function () {
    ...
    it('should return correct out fields, excluding OBJECTID', () => {
      ...
      response.fields
        .find((field) => field.name === 'Dept')
        .name.should.equal('Dept');
  ...
  describe('querying for statistics', function () {
    describe('results are passed in', function () {
      ...
      it('should respect metadata when date field is passed in and custom stat label', () => {
        ...
        response.features[0].attributes.some_new_label.should.equal(
          1497578316179,
        );
        response.fields[0].type.should.equal('esriFieldTypeDate');
      });
    ...
    describe('calculating from geojson', function () {
      ...
      it('should return correct number of fields and features for sum stats', () => {
        ...
        response.fields.length.should.equal(1);
        response.features.length.should.equal(1);
        response.features[0].attributes.sum_precip.should.equal(
          135.69000000000003,
        );
      });
      ...
      it('should return correct number of fields and features for avg stats', () => {
        ...
        response.features[0].attributes.avg_precip.should.equal(
          0.3253956834532375,
        );
      });
          ...
          outStatistics: [
            {
              statisticType: 'var',
              onStatisticField: 'total precip',
              outStatisticFieldName: 'var_precip',
            },
            {
              statisticType: 'stddev',
              onStatisticField: 'total precip',
              outStatisticFieldName: 'stddev_precip',
            },
          ],
        });
        response.features[0].attributes.var_precip.should.equal(
          0.07661480700055341,
        );
        response.features[0].attributes.stddev_precip.should.equal(
          0.27646171244241985,
        );
      });
      ...
      it('should return a correct response when there are multiple stats returned', () => {
        ...
        response.fields
          .findIndex((f) => {
            return f.name === 'Full/Part_COUNT';
          })
          .should.not.equal(-1);
        response.fields
          .findIndex((f) => {
            return f.name === 'Full/Part';
          })
          .should.not.equal(-1);
      });
    });
  ...
  describe('when getting feature counts from a given count', () => {
    it('should return a correct count json', () => {
      const json = FeatureServer.query(
        { count: 100 },
        { returnCountOnly: true },
      );
  ...
  describe('when an offset has already been applied', () => {
    it('should remove the result offset', () => {
      const json = FeatureServer.query(offsetApplied, {
        resultOffset: 50,
        resultRecordCount: 1,
      });
